...
function info_update(changes) {
    ...
    var nr_errors = changes.errors.length;
    var title;
    var txt;
    if (nr_errors == 0) {
        title = 'Remote update succesful';
        txt = '' + (nr_added + nr_modified) + ' files sent, ' +
            nr_deleted + ' files removed.';
    } else {
        title = 'Remote update failed!';
        txt = '' + nr_errors + ' ERRORS, ' +
            (nr_added + nr_modified) + ' files sent, ' +
            nr_deleted + ' files removed.';
    }
...
function refresh() {
    ...
    var sl = SyncList({
        ...
        syncMap: sync_map,
        onComplete: function() {
            env.log('Here we go! Finished!')

            // Make some notifications with the results
            var push_results = this.pushResults;
            info_update(push_results);
