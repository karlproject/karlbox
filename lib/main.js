...
function info_update(pullResults, pushResults, globalConflicts, manual) {
    ...
    if (! manual && push_added == 0 && push_modified == 0 &&
            pull_added == 0 && pull_modified == 0 &&
            push_deleted == 0 && pull_deleted == 0 &&
            push_conflicts == 0 && pull_conflicts == 0 &&
            global_conflicts == 0 && pull_conflicts == 0 &&
            push_errors == 0 && pull_errors == 0) {
        // suppress empty notification
        return;
    }
...
var sl = SyncList({
            ...
            onComplete: function() {
                ...
                info_update(this.pullResults, this.pushResults, this.sync_statuses.conflict, this.manual);

                // Update the panel with the date of the last update
                var now = new Date();
                dbt.worker.postMessage({type: 'last_update',
                            value: now});

                kb_widget.badge = {};

                // Let's check if the file 'agility.json' has been loaded
                //
                var refresh = false;
                this.pullResults.modified.forEach(function(fileName) {
                    if (fileName == dbt.syncFileName) {
                        refresh = true;
                    }
                });
                if (! refresh) {
                    this.pullResults.added.forEach(function(fileName) {
                        if (fileName == dbt.syncFileName) {
                            refresh = true;
                        }
                    });
                ...
                if (refresh) {
                    dbt.worker.postMessage({type: 'sync_changed_file'});
                }
...
function refresh(options) {
    ...
    if (! sl.locked) {
        ...
        // manual status is marked on the synclist
        // ... it it used from update_status to suppress the empty notification
        sl.manual = options.manual;
