...
exports._SyncList = function(options) {
    ...
    this.syncedRemote = ''; // XXX this has to go with sync map
...
exports._SyncList.prototype.queryRemoteChanges = function(callback) {
    var self = this;
    var request = this.plugs.Request({
        url: 'http://localhost:6543/communities/default/files/new_query_file.json',
        content: {
            timestampfrom: this.syncedRemote
        },
        headers: {
        ...
        onComplete: function (response) {
            ...
            if (json_result && json_result.result == "OK") {
                ...
                self.syncedRemote = json_result.timestamp_to;
...
exports._SyncList.prototype.finishUpdateLocal = function (fileNames, json_results) {
    ...
    json_results.forEach(function(item, i) {
        ...
        var map_entry = self.sync_map[fileName];
...
exports._SyncList.prototype.sync = function () {
    ...
    this.queryRemoteChanges(function() {

        self.prepareUpdateLocal();
        ...
        var getter = self.plugs.MultiGetter({
            ...
            onEachResult: function(fileName, response) {
                env.log("Finish pull " + fileName);
                ////self.finishPull(fileName, response);
            },
            onComplete: function() {
                ...
                var poster = self.plugs.MultiPoster({
                    ...
                    onComplete: function(json_results) {
                        if (self.sync_statuses.push.length != json_results.length) {
                            throw new Error('Fatal error in sync');
                        }
                        self.finishUpdateLocal(self.sync_statuses.push, json_results);
