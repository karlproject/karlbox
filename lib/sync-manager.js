...
exports._SyncList.prototype.finishPull = function (syncItem, response) {
    var self = this;
    // We come here if one download is complete
    // (with or without error)
    // We update our sync map accordingly

    var map_entry = syncItem;
    var fileName = syncItem.fileName;
...
exports._SyncList.prototype.finishPush = function (fileName, response) {
    ...
    if (item !== null && item.result == 'OK') {
        ...
        env.log('upload OK', item.status, item.filename);
        ...
        // remember full server path (for download)
        map_entry.serverPath = item.serverPath;
        ...
        env.log('upload CONFLICT', fileName);
        map_entry.status = 'conflict';
        pushStatus = 'conflicts';
    } else if (item !== null && item.result == 'ERROR') {
        // The server told us an error
        env.log('upload ERROR', fileName, 'server said:', item.error);
...
exports._SyncList.prototype.sync = function () {
    ...
    var request = this.plugs.Request({
        ...
        onComplete: function (response) {
            ...
            // for download, we must use the physical paths from the server
            var download_items = [];
            self.sync_statuses.pull.forEach(function(fileName) {
                download_items.push(self.sync_map[fileName]);
            });
            ...
            var getter = self.plugs.MultiGetter({
                ...
                syncItems: download_items,
                onEachResult: function(syncItem, response) {
                    var pull_status = self.finishPull(syncItem, response);
                    self.pullResults[pull_status].push(syncItem.fileName);
                },
                onComplete: function() {
